...
/**
 * The third PUBLIC Machine Learning scenario (Easy) — proves basic text
 * classification (TF-IDF vectorization + a linear classifier, macro F1) end
 * to end on real content. Mirrors `server/scenarios/house-price-regression.test.ts`
 * (regression) and `server/scenarios/iris-species-classifier.test.ts`
 * (numeric classification), and `server/scenarios/customer-churn-classifier.test.ts`
 * (the internal ML reference).
 */
const SLUG = "support-ticket-categorizer";
...
describe("support-ticket-categorizer (public Easy ML text classification scenario)", () => {
  ...
  it("is public: included in every public listing, unlike the internal ML reference", async () => {
    ...
    // The other public Easy ML scenarios are still there too — this task
    // added a third scenario, it didn't replace the first two.
  ...
  it(
    ...
    async () => {
      ...
      expect(preview.stdout).toContain("Loaded training rows: 80");
      expect(preview.stdout).toContain("Loaded test rows: 20");
      expect(preview.stdout).toContain("Prepared text features.");
      ...
      expect(predictions!.preview?.columns).toEqual(["ticket_id", "predicted_category"]);
      expect(predictions!.preview?.rows).toHaveLength(5); // 20 test rows, capped at 5
      expect(predictions!.preview?.truncated).toBe(true);
      const validCategories = ["billing", "technical", "account", "shipping"];
      for (const row of predictions!.preview?.rows ?? []) {
        expect(validCategories).toContain(row.predicted_category);
      }
      ...
      expect(metricsData).toMatchObject({ train_rows: 80, test_rows: 20, model: "LogisticRegression" });
      expect(metricsData.accuracy as number).toBeGreaterThanOrEqual(0.8);
      expect(metricsData.macro_f1 as number).toBeGreaterThanOrEqual(0.75);

      const report = preview.artifacts.find((a) => a.path === "report.txt");
      expect(report).toBeDefined();
      expect(report!.kind).toBe("text");
      expect(report!.preview?.text).toContain("Support Ticket Categorizer Report");
      expect(report!.preview?.text).toContain("Training rows: 80");
