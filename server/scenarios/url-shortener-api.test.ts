...
describe("url-shortener-api backend scenario", () => {
  it("is public as a hard backend scenario", async () => {
  ...
  it("applies and verifies every checkpoint through the production verification path", async () => {
    ...
    const expectedTestCounts = new Map([
      ["create-and-fetch-links", 4],
      ["redirects-and-updates", 5],
      ["click-analytics", 4],
    ]);
