...
/**
 * The first internal Machine Learning reference scenario (Phase 6) — proves ML
 * authoring, workspace loading, Data Preview, step/final verification, and
 * public/internal visibility end to end. `visibility: internal`, so it must
 * never appear in any public listing, but must still load and verify
 * correctly by slug (the same way the dev/internal playground would use it).
 */
const SLUG = "customer-churn-classifier";
...
describe("customer-churn-classifier (internal ML reference scenario)", () => {
  ...
  it("is internal: excluded from every public listing", async () => {
    ...
    expect(summaries.some((s) => s.slug === SLUG)).toBe(false);
    expect(options.some((s) => s.slug === SLUG)).toBe(false);
    expect(candidates.some((s) => s.slug === SLUG)).toBe(false);
    expect(pickerOptions.some((s) => s.slug === SLUG)).toBe(false);
  });

  it("keeps other internal fixtures hidden and backend/fullstack picker visibility unaffected", async () => {
    const options = await listScenarioOptions();
    // Backend/fullstack scenarios are still public — this task changed nothing there.
    expect(options.some((s) => s.slug === "notes-rest-api")).toBe(true);
    expect(options.some((s) => s.slug === "authentication-api")).toBe(true);
    // Pre-existing internal fixture stays hidden too.
    expect(options.some((s) => s.slug === "customer-feedback-dashboard")).toBe(false);
  });

  it("is still loadable by slug for internal/dev flows, with the full ML metadata contract", async () => {
    const loaded = await loadScenario(SLUG, { includeAuthorOnly: true });
    expect(loaded.scenario.visibility).toBe("internal");
  ...
  it(
    ...
    async () => {
      ...
      expect(preview.stdout).toContain("Loaded 60 training rows and 15 test rows.");
      expect(preview.stdout).toContain("Prepared 8 model-ready feature columns.");
      ...
      expect(predictions!.preview?.columns).toEqual(["customer_id", "churn_prediction"]);
      expect(predictions!.preview?.rows).toHaveLength(5); // 15 test rows, capped at 5
      expect(predictions!.preview?.truncated).toBe(true);
      for (const row of predictions!.preview?.rows ?? []) {
        expect(["0", "1"]).toContain(row.churn_prediction);
      }
      ...
      expect(metricsData).toMatchObject({ train_rows: 60, test_rows: 15, model: "DecisionTreeClassifier" });
      expect(metricsData.accuracy as number).toBeGreaterThanOrEqual(0.7);
      expect(metricsData.f1 as number).toBeGreaterThanOrEqual(0);

      const report = preview.artifacts.find((a) => a.path === "report.txt");
      expect(report).toBeDefined();
      expect(report!.kind).toBe("text");
      expect(report!.preview?.text).toContain("Customer Churn Classifier Report");
      expect(report!.preview?.text).toContain("Training rows: 60");
      expect(report!.preview?.text).toContain("Test rows: 15");
    },
    30_000,
  );
