...
/**
 * The second PUBLIC Machine Learning scenario (Easy) — proves ML regression
 * (numeric + categorical preprocessing, MAE/R2 metrics) end to end on real
 * content. Mirrors `server/scenarios/iris-species-classifier.test.ts` (the
 * first public Easy ML scenario, classification) and
 * `server/scenarios/customer-churn-classifier.test.ts` (the internal ML
 * reference).
 */
const SLUG = "house-price-regression";
...
describe("house-price-regression (public Easy ML regression scenario)", () => {
  ...
  it("is public: included in every public listing, unlike the internal ML reference", async () => {
    ...
    // The first public Easy ML scenario is still there too — this task added
    // a second scenario, it didn't replace the first.
  ...
  it(
    ...
    async () => {
      ...
      expect(preview.stdout).toContain("Prepared model-ready feature columns.");
      ...
      expect(predictions!.preview?.columns).toEqual(["home_id", "predicted_price"]);
      ...
      for (const row of predictions!.preview?.rows ?? []) {
        const value = Number(row.predicted_price);
      ...
      expect(metricsData).toMatchObject({ train_rows: 75, test_rows: 20, model: "RandomForestRegressor" });
      expect(metricsData.mae as number).toBeGreaterThanOrEqual(0);
      expect(metricsData.mae as number).toBeLessThanOrEqual(45000);
      expect(metricsData.r2 as number).toBeGreaterThanOrEqual(0.7);

      const report = preview.artifacts.find((a) => a.path === "report.txt");
      expect(report).toBeDefined();
      expect(report!.kind).toBe("text");
      expect(report!.preview?.text).toContain("House Price Regression Report");
