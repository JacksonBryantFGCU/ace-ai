"""Alternative (independently structured) retail demand pipeline implementation.

Same public contract as
content/interview-scenarios/machine-learning-python/retail-demand-forecaster/solution/step-3/src/demand_pipeline.py
(same constant/function names, same chronological-holdout + shifted
lag/rolling features + Pipeline/ColumnTransformer requirements) but
reorganized throughout: helper names, validation control flow, and internal
decomposition diverge from the reference, while the underlying feature and
evaluation formulas are preserved exactly (so the leakage-safety and metrics
thresholds this scenario enforces still hold). Used as a
behavioral-equivalence fixture in server/scenarios/behavioral-equivalence-ml.test.ts
- it must PASS the scenario's real authored pytest suite.
"""
...
def create_lag_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _shifted_rolling_mean(series: pd.Series) -> pd.Series:
        return series.shift(1).rolling(window=7).mean()

    result["rolling_mean_7"] = grouped.transform(_shifted_rolling_mean)
    return result
