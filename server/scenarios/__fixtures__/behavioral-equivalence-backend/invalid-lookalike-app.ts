...
app.post("/transfers", (req: Request, res: Response) => {
  ...
  const runTransfer = db.transaction(() => {
    ...
    // BUG: ledger directions are swapped relative to the money movement -
    // the source account (which loses money) is recorded as a credit and
    // the destination account (which gains money) is recorded as a debit.
    // Balances themselves are still updated correctly, so this only shows
    // up in the ledger entries / double-entry bookkeeping semantics.
    db.run(
