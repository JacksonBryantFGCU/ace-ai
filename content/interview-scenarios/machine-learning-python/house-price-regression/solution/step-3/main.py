"""House Price Regression - entrypoint (reference solution, all steps).

Writes three artifacts next to this file for the Output Preview panel:
  predictions.csv - home_id,predicted_price, one row per test home
  metrics.json    - validation MAE/R2 + run metadata
...
def main() -> None:
    ...
    save_predictions(test_df["home_id"], predictions, str(PREDICTIONS_PATH))
    ...
    report_lines = [
        "House Price Regression Report",
        f"Training rows: {len(train_df)}",
        f"Test rows: {len(test_df)}",
        f"Validation MAE: {mae:.0f}",
        f"Validation R2: {r2:.2f}",
