"""Customer churn pipeline helpers (reference solution, all steps)."""
...
def predict_churn(model: DecisionTreeClassifier, X_test: pd.DataFrame) -> list[int]:
    return [int(value) for value in model.predict(X_test)]


def save_predictions(customer_ids, predictions, path: str) -> None:
    output = pd.DataFrame({"customer_id": list(customer_ids), "churn_prediction": list(predictions)})
