"""Customer Churn Classifier - entrypoint (reference solution, all steps).

Writes three artifacts next to this file for the Output Preview panel:
  predictions.csv - customer_id,churn_prediction, one row per test customer
  metrics.json    - validation accuracy/f1 + run metadata
...
def main() -> None:
    ...
    save_predictions(test_df["customer_id"], predictions, str(PREDICTIONS_PATH))
    ...
    report_lines = [
        "Customer Churn Classifier Report",
