...
app.patch("/orders/:id/status", (req, res) => {
  ...
  if (body.status === undefined) return void res.status(400).json({ error: "Status is required" });
  if (typeof body.status !== "string" || !VALID_STATUSES.has(body.status)) return void res.status(400).json({ error: "Invalid status" });
  if (!TRANSITIONS[order.status]?.includes(body.status)) return void res.status(400).json({ error: "Invalid status transition" });

  db.run("UPDATE orders SET status = ?, updated_at = ? WHERE id = ?", [body.status, nowIso(), id]);
  res.status(200).json({ order: findOrder(id) });
});
