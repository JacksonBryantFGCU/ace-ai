...
import { cancelSubscription, fetchPlans, fetchSubscription, reactivateSubscription, updateSubscription } from "./api";
...
export function App() {
  ...
  async function handleCancel() {
    ...
    try {
      ...
      setCancellationError((err as Error).message);
    } finally {
      setCancelling(false);
    }
  }
    ...
    <main className="app-shell">
      ...
      {payload ? (
        <>
          ...
          <section className="plan-card" aria-label="Current subscription">
            ...
            {payload.subscription.cancel_at_period_end ? (
              <p className="scheduled-note">
                Cancellation scheduled for {formatDate(payload.subscription.current_period_end)}
              </p>
            ) : null}
          </section>
          ...
          <section className="cancellation-panel" aria-label="Cancellation">
            <h2>Cancellation</h2>
            {payload.subscription.status === "cancelled" ? (
              <p className="muted">This subscription is cancelled.</p>
            ) : payload.subscription.cancel_at_period_end ? (
              <button type="button" className="secondary" disabled={cancelling} onClick={handleReactivate}>
                {cancelling ? "Saving..." : "Reactivate subscription"}
              </button>
            ) : (
              <button type="button" className="secondary" disabled={cancelling} onClick={handleCancel}>
                {cancelling ? "Saving..." : "Cancel subscription"}
              </button>
            )}
            {cancellationError ? (
              <p role="alert" className="error">
