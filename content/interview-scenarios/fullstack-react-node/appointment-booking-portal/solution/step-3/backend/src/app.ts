...
const ALLOWED_TARGET_STATUSES = new Set<AppointmentStatus>(["completed", "cancelled"]);
...
app.patch("/appointments/:id/status", async (req, res) => {
  ...
  if (!status || !ALLOWED_TARGET_STATUSES.has(status)) {
    res.status(400).json({ error: "Invalid appointment status" });
    return;
  }

  if (existing.status !== "scheduled") {
    res.status(400).json({ error: "Appointment is already finalized" });
    return;
  }

  const appointment = await updateAppointmentStatus({ id, status, updated_at: new Date().toISOString() });
