...
app.patch("/orders/:id/fulfill", async (req, res) => {
  const id = parseId(req.params.id);
  if (!id) {
    res.status(400).json({ error: "Invalid order id" });
    return;
  ...
  if (existing.status === "fulfilled") {
    res.status(400).json({ error: "Order is already fulfilled" });
    return;
  }
  ...
  const detail = await fulfillOrder(id, new Date().toISOString());
  res.json(detail);
});
...
app.patch("/orders/:id/cancel", async (req, res) => {
  ...
  if (existing.status === "fulfilled") {
    res.status(400).json({ error: "Order is fulfilled" });
    return;
  }

  const detail = await cancelOrder(id, new Date().toISOString());
  res.json(detail);
});
