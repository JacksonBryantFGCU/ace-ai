...
export function App() {
  ...
  function applyUpdatedDetail(updated: OrderDetail) {
    setDetail(updated);
    setOrders((current) => current.map((order) => (order.id === updated.order.id ? updated.order : order)));
  }
  ...
  async function handleFulfill() {
    ...
    setActionError(null);
    try {
      const updated = await fulfillOrder(detail.order.id);
  ...
  async function handleCancel() {
    ...
    setActionError(null);
    try {
      const updated = await cancelOrder(detail.order.id);
      applyUpdatedDetail(updated);
        ...
        <div className="order-detail" aria-label="Order details">
          ...
          {detail ? (
            <>
              ...
              <div className="badge-row">
                <span className={`badge status-${detail.order.status}`}>{statusLabel(detail.order.status)}</span>
              </div>
              ...
              <div className="actions-row">
                <button type="button" disabled={actioning || detail.order.status !== "pending"} onClick={handleFulfill}>
                  {actioning ? "Working..." : "Fulfill order"}
                </button>
                <button
                  type="button"
                  className="danger"
                  disabled={actioning || detail.order.status !== "pending"}
                  onClick={handleCancel}
                >
                  {actioning ? "Working..." : "Cancel order"}
                </button>
              </div>
            </>
          ) : null}
