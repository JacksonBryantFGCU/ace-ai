...
import { createRsvp, fetchEvent, fetchEvents, updateRsvpStatus } from "./api";
...
const RSVP_STATUS_OPTIONS: RsvpStatus[] = ["going", "waitlisted", "cancelled"];
...
export function App() {
  ...
  const [draftRsvpStatus, setDraftRsvpStatus] = useState<Record<number, RsvpStatus>>({});
  const [rsvpItemErrors, setRsvpItemErrors] = useState<Record<number, string>>({});
  ...
  useEffect(() => {
    ...
    fetchEvent(selectedId)
      .then((event) => {
        if (cancelled) return;
        setDetail(event);
        setDraftRsvpStatus(Object.fromEntries(event.rsvps.map((rsvp) => [rsvp.id, rsvp.status])));
      })
      .catch((err: Error) => {
  ...
  async function handleRsvpSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDraftRsvpStatus((current) => ({ ...current, [result.rsvp.id]: result.rsvp.status }));
  ...
  async function handleRsvpStatusUpdate(rsvpId: number) {
    setSavingRsvpId(rsvpId);
    setRsvpItemErrors((current) => ({ ...current, [rsvpId]: "" }));
    try {
      const nextStatus = draftRsvpStatus[rsvpId];
      if (!nextStatus) return;
      const result = await updateRsvpStatus(rsvpId, nextStatus);
      setDetail((current) =>
        current
          ? {
              ...result.event,
              rsvps: current.rsvps.map((rsvp) => (rsvp.id === result.rsvp.id ? result.rsvp : rsvp)),
            }
          : current,
      );
      setDraftRsvpStatus((current) => ({ ...current, [result.rsvp.id]: result.rsvp.status }));
      ...
      setRsvpItemErrors((current) => ({ ...current, [rsvpId]: (err as Error).message }));
    } finally {
                  ...
                  {detail.rsvps.map((rsvp) => (
                    <li className="rsvp-row" key={rsvp.id}>
                      <div className="rsvp-row-info">
                        ...
                        {rsvpItemErrors[rsvp.id] ? (
                          <p role="alert" className="error">{rsvpItemErrors[rsvp.id]}</p>
                        ) : null}
                      </div>

                      <span className={`status status-${rsvp.status}`}>{statusLabel(rsvp.status)}</span>

                      <label htmlFor={`rsvp-status-${rsvp.id}`}>Update status for {rsvp.attendee_name}</label>
                      <select
                        id={`rsvp-status-${rsvp.id}`}
                        value={draftRsvpStatus[rsvp.id] ?? rsvp.status}
                        onChange={(event) =>
                          setDraftRsvpStatus((current) => ({
                            ...current,
                            [rsvp.id]: event.target.value as RsvpStatus,
                          }))
                        }
                      >
                      ...
                      <button
                        ...
                        onClick={() => handleRsvpStatusUpdate(rsvp.id)}
                      >
                        {savingRsvpId === rsvp.id ? "Saving..." : "Update"}
                      </button>
                    </li>
                  ))}
                </ul>
